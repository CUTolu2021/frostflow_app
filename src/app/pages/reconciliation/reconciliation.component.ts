...
export class ReconciliationComponent implements OnInit {
    ...
    async trustOwner(item: any) {
        if (
            confirm(`Update inventory to OWNER count (${item.owner_quantity})?`)
        ) {
            await this.processResolution(
                item,
                item.owner_quantity,
                'Owner count accepted'
            )
        }
    }
    ...
    async trustSales(item: any) {
        ...
        if (
            confirm(
                `Update inventory to SALES count (${salesQty})? This confirms a loss of ${item.difference}.`
            )
        ) {
            await this.processResolution(
                item,
                salesQty,
                'Sales count accepted (Loss confirmed)'
            )
        }
    }

    async manualFix(item: any) {
        const finalVal = prompt(
